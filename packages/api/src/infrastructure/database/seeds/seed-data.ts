...
const teamMembersData: Omit<Partial<TeamMemberEntity>, 'passwordHash'>[] = [
    ...
    { id: 'tm_020', firmId: 'firm_001', name: 'Kenny Song', email: 'kenny@doublehq.com', role: TeamRole.MANAGER, avatarUrl: 'https://ui-avatars.com/api/?name=Kenny+Song&background=059669&color=fff', isActive: true },
...
export async function runSeed(dataSource: DataSource, password: string): Promise<string> {
    logger.info('🌱 Starting seed (via API endpoint)...');
    ...
    let tmplIdx = 0, tsIdx = 0, ttIdx = 0;

    for (const f of firms) {
        tmplIdx++;
        const tmplId = `tmpl_${String(tmplIdx).padStart(3, '0')}`;
        await manager.save(CloseTemplateEntity, { id: tmplId, firmId: f.id!, name: 'Standard Monthly Close', isDefault: true });
        ...
        for (const sec of templateSections) {
            ...
            await manager.save(TemplateSectionEntity, { id: tsId, templateId: tmplId, name: sec.name, order: sec.order });
        }
    ...
    const summary = `✅ Seed complete: ${firms.length} firms, ${teamMembersData.length} team members, ${firms.length} templates`;
    logger.info(summary);
    return summary;
}
